...
class Enricher:
    """丰富节点，负责为已整理的文档获取原始内容。

    并行获取多个 URL 的原始内容，通过 Tavily API 提取网页正文。
    """

    def __init__(self) -> None:
        """初始化 Enricher。

        初始化 Tavily 客户端和批次大小设置。
        """
    ...
    async def run(self, state: ResearchState) -> ResearchState:
        ...
        Returns:
            ResearchState: 更新后的研究状态，出错时返回原状态
        """
        try:
