...
class GroundingNode:
    ...
    该节点通过爬取公司网站和收集基本信息，为后续研究提供基础数据。
    """

    def __init__(self) -> None:
        """初始化 GroundingNode。

        初始化 Tavily 客户端用于后续的网络爬取操作。
        """
    ...
    async def initial_search(self, state: InputState):
        ...
        该方法执行以下操作：
        1. 初始化研究任务并发送事件
        2. 如果提供了公司 URL，则爬取公司网站
        3. 收集公司总部位置和行业信息
        4. 返回包含初始研究状态的 ResearchState

        Args:
            state: 输入状态，包含公司名称、URL、行业和总部位置等信息
    ...
    async def run(self, state: InputState) -> ResearchState:
        ...
        执行初始搜索流程，返回包含公司信息和爬取内容的 ResearchState。

        Args:
            state: 输入状态

        Returns:
            ResearchState: 包含公司网站内容和上下文信息的研究状态
        """
